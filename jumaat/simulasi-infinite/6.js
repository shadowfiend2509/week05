...
function changeXRecursive(data, jenis) {
  ...
  if(data.length===1){
    ...
    var angka = Number(data[0])
    if(angka === 0){
      return angka + changeXRecursive(data.slice(1))
    }
    else if(jenis === 'genap'){
      if(angka ===2 || angka === 4 || angka === 6 || angka === 8){
        var hasil =''
        return (Math.round(10 - Number(angka))).toString() + changeXRecursive(data.slice(1))
      }
    }
