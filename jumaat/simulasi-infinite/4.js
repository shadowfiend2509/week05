...
function train(pokemon, candy) {
  // code here
  var naik = pokemon.level + candy
  var nama = []

  if(!pokemon.evolutionLine){
    return 'Congrastilation, your '+pokemon.name+' grew to LV. '+naik
  }
  for(var i=0; i<pokemon.evolutionLine.length; i++){
    if(naik >= pokemon.evolutionLine[i].level){
      nama.push(pokemon.evolutionLine[i].name)
    }
  }

    return 'Congrastulation, your '+pokemon.name+' grew to LV. '+naik+'! and to evolved into '+nama
  

  // var hasil =''
  // for(var i=0; i<pokemon.evolutionLine.length; i++){
  //   var plus = 0
  //   // for(var j=0; j<pokemon.evolutionLine[i].length; j++){
  //     if(naik >= pokemon.evolutionLine[i].level){
  //       hasil+= 'Congratulation, your '+pokemon+' grew to LV. '+pokemon.evolutionLine[i].level.toString()+'! And to evolved into '+pokemon.evolutionLine[i].name
  //       plus++
  //     }
